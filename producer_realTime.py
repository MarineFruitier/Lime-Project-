...
def Velib_realTime_api():
    #Initialisation des variables
    current_page =0
    current_offset = 0
    total_result = getTotalResult() #Activer cette option si on veut récupérer les résultats de toutes les bornes. Attention grosse volumétrie.
    ...
    while current_page < total_result:
        ...
        for result in results:
            ...
            mechanical = str(result.get('mechanical'))
            ebike = str(result.get('ebike'))
            # Vérifier si toutes les valeurs nécessaires sont présentes
            if all([stationcode, mechanical, ebike]):
                results_value = {
...
try:
    while True:
        record_key = "Velib_RealTime"
        record_value = Velib_realTime_api()
